...
const toPositiveInt = (value: number, fallback: number, max?: number) => {
...
const normalizeUuidList = (values: string[]) =>
  Array.from(
    new Set(
      values
...
const normalizeRadiusKm = (value: string) => {
...
const pickParamValue = (value: string | string[] | undefined, maxLength = 255) => {
...
export const parseMobileSearchRouteParams = (params: SearchRouteParams) => {
...
export const buildListingsSearchParams = (args: {
  page: number
  limit: number
  term?: string
  selectedCity?: string
